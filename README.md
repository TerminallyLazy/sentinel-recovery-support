...
Submit one public Node.js or TypeScript repository URL, one public failing CI log
URL, a reply email, and an optional failure summary. No sign-in or meeting is
required. The request moves no funds, authorizes no payment, and starts no work;
a separately human-approved written SOW is required before payment or work.
Google Forms processes the submitted public URLs and reply email privately for
the form owner.
